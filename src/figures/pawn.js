...
export class Pawn extends Figure {
  ...
  canBeat(startCell, endCell, dx, dy) {
    if(this.isEnemyFigure(startCell, endCell) && dx == 1 && Math.abs(dy) == 1 ) {
      return true;
    }
    return false;
  }
  ...
  canDoMove(startCell, endCell, dx, dy) {
    if(startCell.figure.color === COLORS.white && dy < 0 && !endCell.figure) {
      ...
      return dx == 0 && dy == -1 || dx == 0 && dy == -START_POSITION.doubleMovePawn && startCell.figure.isFirstMove;
    } else if(startCell.figure.color !== COLORS.white && dy > 0 && !endCell.figure){
