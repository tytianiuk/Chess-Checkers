...
export class Checker extends Figure {
  ...
  canMove(startCell, endCell) {
    ...
    if(startCell.board.canMultiBeat === null) {
      if(Math.abs(dx) === START_POSITION.moveLengthChecker 
      && Math.abs(dy) === START_POSITION.moveLengthChecker) {
        return this.canDoMove(startCell, endCell, dy);
      }

      if(Math.abs(dx) === START_POSITION.jumpLengthChecker 
  ...
  canBeat(startCell, endCell, dx, dy) {
    if(Math.abs(dx) !== START_POSITION.jumpLengthChecker 
    || Math.abs(dy) !== START_POSITION.jumpLengthChecker) return false;
    ...
    if(startCell.figure.color !== cell.figure.color && this.isFreeCell(endCell)) {
      cell.figure = null;
      return true;
    } 
    return false;
  ...
  canBeatInAnyDirection(cell) {
    const directions = [
      { dx: START_POSITION.jumpLengthChecker, dy: START_POSITION.jumpLengthChecker }, 
      { dx: -START_POSITION.jumpLengthChecker, dy: START_POSITION.jumpLengthChecker }, 
      { dx: START_POSITION.jumpLengthChecker, dy: -START_POSITION.jumpLengthChecker },
      { dx: -START_POSITION.jumpLengthChecker, dy: -START_POSITION.jumpLengthChecker } 
    ];
    ...
    for (const direction of directions) {
      ...
      if (cellForJumpX >= 0 && cellForJumpX < START_POSITION.size && 
          cellForJumpY >= 0 && cellForJumpY < START_POSITION.size) {
