...
export class King extends Figure {
  ...
  castle(startCell, endCell) {
    ...
    if(startCell.x - endCell.x < 0) {
      this.cells[startCell.y][startCell.x + START_POSITION.castleJumpKing].figure = startCell.figure;
      this.cells[startCell.y][startCell.x + START_POSITION.castleJumpRook].figure = endCell.figure;
    } else {
      this.cells[startCell.y][startCell.x - START_POSITION.castleJumpKing].figure = startCell.figure;
      this.cells[startCell.y][startCell.x - START_POSITION.castleJumpRook].figure = endCell.figure;
    }
  ...
  canCastle(startCell, endCell){
    if (!startCell.figure.isFirstMove || !endCell.figure.isFirstMove) return false;
